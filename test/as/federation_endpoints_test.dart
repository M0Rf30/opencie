...
void main() {
  group('Federation Endpoints', () {
    ...
    test(
      '/.well-known/openid-federation returns signed JWT entity configuration',
      () async {
        final base = idp.baseUrl!;
        final federationUri = base.replace(
        ...
        expect(
          parts.length,
          3,
          reason: 'JWT should have 3 parts (header.payload.signature)',
        );

        // Decode header
        final headerJson =
            json.decode(utf8.decode(base64Url.decode(_pad(parts[0]))))
        ...
        expect(opMetadata['token_endpoint_auth_methods_supported'], [
          'private_key_jwt',
        ]);
        ...
        final trustMark =
            (payloadJson['trust_marks'] as List).first as Map<String, dynamic>;
        expect(
          trustMark['id'],
          'https://registry.example.it/openid_provider/public/',
        );
    ...
    test(
      '/federation/trust_mark_status returns active:true for valid trust mark',
      () async {
        final base = idp.baseUrl!;
    ...
    test(
      '/federation/trust_mark_status returns active:false for invalid trust mark',
      () async {
        final base = idp.baseUrl!;
        final statusUri = base.replace(
          path: '/federation/trust_mark_status',
          queryParameters: {'trust_mark_id': 'bogus', 'sub': 'bogus'},
        );
