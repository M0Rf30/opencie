...
void main() {
  group('DER encoding/decoding', () {
    ...
    test('algorithmIdentifier for sha256WithRSA', () {
      final algId = algorithmIdentifier(Oid.sha256WithRSA, parameters: ASN1Null());
    ...
    test('hashOf supports SHA-1 (required for OCSP CertID)', () {
      ...
      final expected = Uint8List.fromList([
        0x70, 0x37, 0x80, 0x71, 0x98, 0xc2, 0x2a, 0x7d, 0x2b, 0x08,
        0x07, 0x37, 0x1d, 0x76, 0x37, 0x79, 0xa8, 0x4f, 0xdf, 0xcf,
      ]);
  ...
  group('Oid.hashForSignatureAlgorithm', () {
    test('maps RSA signature algorithms to hash OIDs', () {
      expect(Oid.hashForSignatureAlgorithm(Oid.sha256WithRSA), equals(Oid.sha256));
      expect(Oid.hashForSignatureAlgorithm(Oid.sha384WithRSA), equals(Oid.sha384));
      expect(Oid.hashForSignatureAlgorithm(Oid.sha512WithRSA), equals(Oid.sha512));
    });

    test('maps ECDSA signature algorithms to hash OIDs', () {
      expect(Oid.hashForSignatureAlgorithm(Oid.ecdsaWithSha256), equals(Oid.sha256));
      expect(Oid.hashForSignatureAlgorithm(Oid.ecdsaWithSha384), equals(Oid.sha384));
      expect(Oid.hashForSignatureAlgorithm(Oid.ecdsaWithSha512), equals(Oid.sha512));
    });
