...
Uint8List buildSyntheticCadesBes({
  ...
  messageDigestAttrValues.add(ASN1OctetString(octets: Uint8List(32))); // Dummy SHA-256 hash
...
void main() {
  group('CadesParser', () {
    ...
    test('setUnsignedAttribute on signer with NO existing unsignedAttrs', () {
      ...
      dummyAttrValueSet.add(ASN1OctetString(octets: Uint8List.fromList([1, 2, 3])));
      final dummyAttrValueSetDer = derEncode(dummyAttrValueSet);
      
      sd.setUnsignedAttribute(Oid.signatureTimeStampToken, dummyAttrValueSetDer);
      final encodedDer = sd.encode();
      
      // Re-parse and verify the attribute is present
      final sd2 = CadesSignedData.parse(encodedDer);
      final retrievedAttr = sd2.getUnsignedAttribute(Oid.signatureTimeStampToken);
    ...
    test('setUnsignedAttribute on signer WITH existing unsignedAttrs', () {
      ...
      dummyAttrValueSet1.add(ASN1OctetString(octets: Uint8List.fromList([1, 2, 3])));
      ...
      dummyAttrValueSet2.add(ASN1OctetString(octets: Uint8List.fromList([4, 5, 6])));
      ...
      final sd2 = CadesSignedData.parse(encodedDer);
      final retrievedAttr1 = sd2.getUnsignedAttribute(Oid.signatureTimeStampToken);
    ...
    test('setUnsignedAttribute REPLACES existing attr with same OID', () {
      ...
      final dummyAttrValueSet2 = ASN1Set();
      dummyAttrValueSet2.add(ASN1OctetString(octets: Uint8List.fromList([7, 8, 9])));
      final dummyAttrValueSetDer2 = derEncode(dummyAttrValueSet2);
      
      sd.setUnsignedAttribute(Oid.signatureTimeStampToken, dummyAttrValueSetDer2);
      final encodedDer = sd.encode();
    ...
    test('getUnsignedAttribute returns the right bytes; null when absent', () {
      ...
      final unsignedAttrs = {
        Oid.signatureTimeStampToken: dummyAttrValueSetDer,
      };
      
      final originalDer = buildSyntheticCadesBes(unsignedAttrs: unsignedAttrs);
      final sd = CadesSignedData.parse(originalDer);
      
      final retrievedAttr = sd.getUnsignedAttribute(Oid.signatureTimeStampToken);
      expect(retrievedAttr, isNotNull);
      expect(retrievedAttr, equals(dummyAttrValueSetDer));
     ...
     test('embeddedCertificates lists the right count', () {
       ...
       final originalDer = buildSyntheticCadesBes(embeddedCerts: [dummyCertDer, dummyCertDer]);
       final sd = CadesSignedData.parse(originalDer);
     ...
     test('parse TRUE IMPLICIT signedAttrs (no inner SET wrapper)', () {
       ...
       final implicitAttrsBytes = Uint8List.fromList([...attr1Bytes, ...attr2Bytes]);
     ...
     test('encapContentInfoForAtsV3 returns only OID TLV for detached content', () {
     ...
     test('multi-signer CMS is rejected', () {
       ...
       contentTypeAttr1.add(ASN1ObjectIdentifier.fromIdentifierString(Oid.contentType));
       final contentTypeAttrValues1 = ASN1Set();
       contentTypeAttrValues1.add(ASN1ObjectIdentifier.fromIdentifierString(Oid.pkcs7Data));
       contentTypeAttr1.add(contentTypeAttrValues1);
       signedAttrs1.add(contentTypeAttr1);
       
       final messageDigestAttr1 = ASN1Sequence();
       messageDigestAttr1.add(ASN1ObjectIdentifier.fromIdentifierString(Oid.messageDigest));
       ...
       contentTypeAttr2.add(ASN1ObjectIdentifier.fromIdentifierString(Oid.contentType));
       final contentTypeAttrValues2 = ASN1Set();
       contentTypeAttrValues2.add(ASN1ObjectIdentifier.fromIdentifierString(Oid.pkcs7Data));
       contentTypeAttr2.add(contentTypeAttrValues2);
       signedAttrs2.add(contentTypeAttr2);
       
       final messageDigestAttr2 = ASN1Sequence();
       messageDigestAttr2.add(ASN1ObjectIdentifier.fromIdentifierString(Oid.messageDigest));
