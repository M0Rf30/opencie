...
void main() {
  group('CadesLtUpgrader', () {
    test(
      'upgrade adds cert-values + rev-values to a synthetic BES with no unsigned attrs',
      () {
    ...
    test(
      'dedup: cert already in SignedData.certificates is NOT in cert-values',
      () {
        final dummyCert = ASN1Sequence();
        dummyCert.add(ASN1Integer(BigInt.from(1)));
        final dummyCertDer = derEncode(dummyCert);
    ...
    test('only OCSP (no CRLs) → revocation-values has only [1] ocspVals', () {
      ...
      final material = ValidationMaterial(ocspResponses: [ocspResponse]);

      final upgradedDer = upgrader.upgrade(originalDer, material);

      // Re-parse and verify revocation-values is present
      final sd = CadesSignedData.parse(upgradedDer);
      final revocationValuesAttr = sd.getUnsignedAttribute(
      ...
      final revocationValuesSeq = derDecode(
        derEncode((revocationValuesSet as ASN1Set).elements![0]),
      );
      expect(revocationValuesSeq, isA<ASN1Sequence>());

      // Check for [1] EXPLICIT tag
      bool hasOcspVals = false;
      if (revocationValuesSeq is ASN1Sequence &&
          revocationValuesSeq.elements != null) {
    ...
    test('only CRLs (no OCSP) → revocation-values has only [0] crlVals', () {
      ...
      final material = ValidationMaterial(crls: [crlData]);
    ...
    test('upgrade preserves existing unsigned timestamp attr', () {
      ...
      dummyTimestampAttrValueSet.add(
        ASN1OctetString(octets: Uint8List.fromList([1, 2, 3])),
      );
      final dummyTimestampAttrValueSetDer = derEncode(
        dummyTimestampAttrValueSet,
      );
      ...
      final sd = CadesSignedData.parse(upgradedDer);
      final timestampAttr = sd.getUnsignedAttribute(
